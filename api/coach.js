// api/coach.js — Unified chat (general + practice + small-talk), CORS, fallbacks
export const config = { runtime: "edge" };

/* ================= Prompt for practice/small-talk ================= */
...
- If correct: praise briefly and optionally invite a follow-up.
...
Mode "chat": reply to small talk in one kind sentence, then segue back to the current prompt. Use "verdict":"unsure".
`;
...
export default async function handler(req) {
  ...
  try {
    ...
    const {
      mode = "auto",          // "auto" | "general" | "eval" | "chat"
    ...
    /* ---------- General ChatGPT mode ---------- */
    // Chosen explicitly OR implied when target/task missing.
    if (mode === "general" || (!target && !task)) {
      if (!process.env.OPENAI_API_KEY) {
        return json({ assistant: "Hi! (general chat is offline on this deploy)", verdict: "chat", explanation: "" }, 200, req);
      }
      const res = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { role: "system",
              content: "You are a warm, concise assistant. Reply in 1–5 sentences. Use markdown when helpful. ALWAYS return JSON only: {\"assistant\":\"...\",\"verdict\":\"chat\",\"explanation\":\"\"}." },
      ...
      if (!res.ok) {
        return json({ assistant: "Sorry — general chat is unavailable right now.", verdict: "chat", explanation: "" }, 200, req);
      }
    ...
    /* ---------- Practice / Small-talk (needs task+target) ---------- */
    if (!task || !target || !user_input) {
      return json({ assistant: "Missing task/target/user_input.", verdict: "unsure", explanation: "" }, 400, req);
    }

    // Offline fallback keeps UI responsive
    if (!process.env.OPENAI_API_KEY) {
      const out = (mode === "chat")
    ...
    // Few-shots to set tone + JSON discipline
    const shots = [
...
function segues(t,task){ return task==="sentence" ? `Let’s keep going: try using “${t}” in a sentence.` : `Your turn — which word matches the definition for “${t}”?`; }
...
function offlineEvaluate({ task, target, definition, user_input, alternatives=[] }){
  if (task==="sentence"){
    ...
    return ok ? { assistant:"Nice — that sounds natural. Ready for the next one?", verdict:"correct", explanation:"" }
              : { assistant:`Almost — try using “${target}” directly. Hint: starts with “${(target||"")[0]||""}”, ${wc(target)} word(s). Another try?`, verdict:"incorrect", explanation:"" };
  } else {
    const guess=norm(user_input), tgt=norm(target), alts=(alternatives||[]).map(norm);
    const ok = (guess===tgt) || alts.includes(guess);
    return ok ? { assistant:`Great, it’s “${target}”. Want to put it in a sentence?`, verdict:"correct", explanation:"" }
              : { assistant:`Not quite. Hint: starts with “${(target||"")[0]||""}”, ${wc(target)} word(s). Another guess?`, verdict:"incorrect", explanation:"" };
  }
}
function safeJSON(s){ try{return JSON.parse(s);}catch{} const a=s.indexOf("{"), b=s.lastIndexOf("}"); if(a>=0&&b>a){ try{return JSON.parse(s.slice(a,b+1));}catch{} } return null; }
